...
def transformation_u_to_omega(u_vector, theta_frank, theta_plackett):

    standard_normal = st.norm(loc=0, scale=1)
    ind_vector = standard_normal.cdf(u_vector)
    ind_vector_c_phi = ind_vector[:, :2]
    ind_vector_z_r = ind_vector[:, 2:4]
    dep_vector = np.zeros((u_vector.shape[0], u_vector.shape[1]))

    dep_vector[:, 0], dep_vector[:, 1] = rvs_frank(ind_vector_c_phi,
                                                   theta_frank)
    dep_vector[:, 2], dep_vector[:, 3] = rvs_plackett(ind_vector_z_r,
                                                      theta_plackett)
    dep_vector[:, 4] = ind_vector[:, 4]
    dep_vector[:, 5] = ind_vector[:, 5]
