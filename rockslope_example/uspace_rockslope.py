...
import rockslope_example.ospace_rockslope as Ospace
import rockslope_example.xspace_rockslope as Xspace
# from ospace_example1 import transformation_u_to_omega
# from xspace_example1 import transformation_omega_to_x, failure_prob


def mma(theta0, g0, N, b, spread=2, bound='upper', g_failure='>=', Tfrank=-10,
        Tplackett=0.1, T=0):
...
def SubSim(d=6, b=0, N=1000, p0=0.1, bound='upper', g_failure='>=0', Tfrank=10,
           Tplackett=0.1, T=0):
        ...
        for k in range(Nc):  # Nc = N*p0 = number of chains
            theta_from_seed[:, :, k], g_from_seed[:, k] = \
                mma(tseed[k, :], gseed[k], Ns, b_j[j], spread=2, bound=bound,
                    g_failure=g_failure, Tfrank=Tfrank, Tplackett=Tplackett,
                    T=T)
