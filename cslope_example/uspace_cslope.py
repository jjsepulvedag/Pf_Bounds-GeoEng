...
import cslope_example.ospace_cslope as Ospace
import cslope_example.xspace_cslope as Xspace
# from ospace_example1 import transformation_u_to_omega
# from xspace_example1 import transformation_omega_to_x, failure_prob


def mma(theta0, g0, N, b, spread=2, bound='upper', g_failure='>=', tn16=0.01):
    ...
    for i in range(N - 1):
        ...
        gg = sign_g*Xspace.failure_prob(x_sample, bound=bound)
        if gg > b:  # eq. 10
...
def SubSim(d=2, b=0, N=1000, p0=0.1, bound='upper', g_failure='>=0', tn16=0.01):
    ...
    g[:, j] = sign_g*Xspace.failure_prob(x_samples, bound=bound)
